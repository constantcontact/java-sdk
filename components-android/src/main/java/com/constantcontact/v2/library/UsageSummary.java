...
public class UsageSummary implements Parcelable {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeInt(_folderCount);
        dest.writeInt(_fileCount);
        dest.writeInt(_imageCount);
        dest.writeInt(_documentCount);
        dest.writeInt(_totalBytesUsed);
        dest.writeInt(_imageBytesUsed);
        dest.writeInt(_documentBytesUsed);
        dest.writeInt(_totalBytesRemaining);
        dest.writeInt(_freeFilesRemaining);
    }

    protected UsageSummary(Parcel in) {
        _folderCount = in.readInt();
        _fileCount = in.readInt();
        _imageCount = in.readInt();
        _documentCount = in.readInt();
        _totalBytesUsed = in.readInt();
        _imageBytesUsed = in.readInt();
        _documentBytesUsed = in.readInt();
        _totalBytesRemaining = in.readInt();
        _freeFilesRemaining = in.readInt();
    }

    public static final Parcelable.Creator<UsageSummary> CREATOR = new Parcelable.Creator<UsageSummary>() {
        @Override
        public UsageSummary createFromParcel(Parcel source) {
